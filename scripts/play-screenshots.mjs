 ...
 *   yarn screenshots
 *   SCREENSHOT_THEME=light yarn screenshots
 ...
 * the map viewport is a persisted setting, so a good screenshot needs no clicking
 * — just the right link plus a seeded view. `#v=lat,lng,zoom` is read by the init
 * script below; the app itself ignores the hash.
 *
 * Zooms are chosen against the layer thresholds: 16 is where individual vehicles
 * reach full opacity, 13 is where the cluster discs take over.
 */
const SHOTS = [
  { center: DOWNTOWN, name: '1-map', url: '/', zoom: 16 },
  {
    center: DOWNTOWN,
    name: '2-route',
    url: `/?route=${process.env.SCREENSHOT_ROUTE_ID ?? '11'}`,
    zoom: 13,
  },
  { center: DOWNTOWN, name: '3-clusters', url: '/', zoom: 13 },
  { center: DOWNTOWN, name: '4-train', url: '/train', zoom: 12 },
  { center: DOWNTOWN, name: '5-cycling', url: '/cycling', zoom: 15 },
  { center: DOWNTOWN, name: '6-city', url: '/city', zoom: 15 },
];
  ...
  try {
    for (const device of DEVICES) {
      ...
      for (const shot of SHOTS) {
        ...
        const file = path.join(OUT_DIR, `${device.name}-${shot.name}.png`);
...
function seedScript(theme) {
  ...
  const base = { globalOnboardingCompleted: true, onboardingCompleted, theme };
