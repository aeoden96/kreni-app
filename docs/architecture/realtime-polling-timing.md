...
| `TTL`      | edge cache TTL (`CACHE_TTL_SECONDS`)          | Worker            | **7 s**               | `s-maxage` + `max-age`, `index.ts`                     |
| `P`        | `REALTIME_POLL_INTERVAL` (nominal target)     | client            | **7000 ms**           | `config/index.ts`                                      |
...
## Current — single ping (7 s config)
...
## Proposed — single ping (10 s aligned)

Two options; **B is recommended.** Both set `P = 10000` and `TTL = 10` at minimum.
...
|                       | Current (7 s)   | Option A (10 s) | Option B (phase-locked)      |
| --------------------- | --------------- | --------------- | ---------------------------- |
| Nominal `P` / `TTL`   | 7 s / 7 s       | 10 s / 10 s     | 10 s / dynamic to regen      |
| Steady-state loop     | ~8.5 s          | ~11.5 s         | ~10 s (locked)               |
| Duplicate ZET fetches | ~1 in 7         | rare            | ~none                        |
| Worst-case data age   | ~17 s + latency | ~13 s + latency | ~10 s + latency              |
| Requests vs today     | —               | ~−30 %          | ~−30 %                       |
| Change size           | —               | one-liner ×2    | Worker logic + client buffer |
...
- [ ] Keep `P` and `TTL` **equal** (Option A) or `TTL` dynamic (Option B); a `TTL < P` gap re-introduces the wasted-HIT thrash the buffer was added to fix.
