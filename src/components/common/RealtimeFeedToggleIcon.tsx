 ...
 */
export function RealtimeFeedToggleIcon({ lastUpdate, open }: RealtimeFeedToggleIconProps) {
  const activityClass = [
  ...
  return (
    <span className="contents" key={lastUpdate ?? 'none'}>
      <Activity aria-hidden className={activityClass} strokeWidth={2.25} />
      <span
        aria-hidden
