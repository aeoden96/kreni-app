...
import type { ParkingZoneProperties, ZoneInfo, SubzoneInfo } from '../../types/parkingZones';
...
export function ParkingZoneModal({ zone, zoneInfo, onClose }: ParkingZoneModalProps) {
    ...
    const subInfo: SubzoneInfo | null =
        ...
        >
            ...
            <div className="p-3 border-b border-base-300 flex-shrink-0">
                <div className="flex items-center gap-2">
                    ...
                    <button onClick={onClose} className="btn btn-ghost btn-circle btn-xs flex-shrink-0">
                        <X className="w-4 h-4" />
                    </button>
                </div>
            </div>
                    ...
                    <div className="divide-y divide-base-300">
                        {displayInfo.enforcement.weekday && (
                            <div className="flex justify-between items-center px-3 py-2">
                                ...
                                <span className="text-xs font-semibold text-base-content">{displayInfo.enforcement.weekday}</span>
                            </div>
                        )}
                        {displayInfo.enforcement.saturday && (
                            <div className="flex justify-between items-center px-3 py-2">
                                <span className="text-xs text-base-content/70">Subota</span>
                                <span className="text-xs font-semibold text-base-content">{displayInfo.enforcement.saturday}</span>
                            </div>
                        )}
                        {displayInfo.enforcement.sunday && (
                            <div className="flex justify-between items-center px-3 py-2">
                                <span className="text-xs text-base-content/70">Nedjelja / praznik</span>
                                <span className="text-xs font-semibold text-base-content">{displayInfo.enforcement.sunday}</span>
                            </div>
                        )}
                    </div>
                </div>
                    ...
                    {displayInfo.maxTime && (
                        <div className="rounded-lg overflow-hidden border border-base-300">
                            ...
                            <div className="px-2 py-2">
                                <span className="text-xs font-semibold text-base-content">{displayInfo.maxTime}</span>
                            </div>
                        </div>
                    )}

                    {displayInfo.prices.length > 0 && (
                        <div className={`rounded-lg overflow-hidden border border-base-300 ${!displayInfo.maxTime ? 'col-span-2' : ''}`}>
                            <div className="px-2 py-1.5 bg-base-200/60 text-[10px] font-bold uppercase tracking-wider text-base-content/50">
                                Cijena karte
                            </div>
                            <div className="divide-y divide-base-300">
                ...
                {zone.streets.length > 0 && (
                    <div className="rounded-lg overflow-hidden border border-base-300">
                        ...
                        >
                            ...
                            {isStreetsOpen ? <ChevronUp className="w-3 h-3" /> : <ChevronDown className="w-3 h-3" />}
                        </button>
                        {isStreetsOpen && (
                            <ul className="divide-y divide-base-300 max-h-48 overflow-y-auto">
                                {zone.streets.map((street, i) => (
                                    <li key={i} className="px-3 py-1.5 text-xs text-base-content/70">
                                        {street}
                                    </li>
                                ))}
                            </ul>
