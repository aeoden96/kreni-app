  ...
  return (
    <>
      {points.map((pt) => (
        ...
        >
          <Tooltip className="congestion-tooltip" direction="top" offset={[0, -8]}>
            <div className="text-xs space-y-0.5">
              <div className="font-bold">{pt.stopName}</div>
              <div>{formatDelay(pt.avgDelay)}</div>
              {pt.maxDelay > 60 && (
                <div className="text-base-content/60">max: {Math.round(pt.maxDelay / 60)} min</div>
              )}
...
function formatVsHistorical(ratio: number | undefined): null | string {
