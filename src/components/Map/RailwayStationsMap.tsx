...
export const RailwayStationsMap = memo(function RailwayStationsMap({
  show,
}: RailwayStationsMapProps) {
        ...
        return (
          ...
          >
            <Tooltip className="custom-tooltip shadow-lg !p-3" direction="top" offset={[0, -16]}>
              ...
              {props?.Vrsta && <div className="text-sm text-base-content/70">{props.Vrsta}</div>}
              {props?.Opis && <div className="text-xs text-base-content/60 mt-1">{props.Opis}</div>}
            </Tooltip>
          </Marker>
        );
      })}
