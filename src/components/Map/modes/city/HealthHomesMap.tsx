...
export const HealthHomesMap = memo(function HealthHomesMap({ show }: Props) {
  const { bounds, shouldRenderDetail } = useStaticLayerRenderGate({ variant: 'city' });
  const geoData = useStaticDatasetJson<FeatureCollection<Point>>('health-homes.geojson', show, {
    logErrorLabel: 'health homes',
  });
  ...
  const featuresForCluster = useMemo(() => {
    ...
    return annotateCityClusterFeatures(valid, 'healthHomes');
  ...
  const icon = L.divIcon({
    className: 'health-home-icon',
    html: `<div class="flex items-center justify-center w-7 h-7 rounded-full bg-blue-500 border-2 border-white shadow-lg text-[13px]">🏥</div>`,
    iconAnchor: [14, 14],
    iconSize: [28, 28],
  });
  ...
  return (
    <>
      {visibleFeatures.map((f, i) => {
        ...
        const title =
          firstStringProp(p, ['Domovi_zdravlja_naziv', 'naziv', 'NAZIV']) || 'Dom zdravlja';
        const address = firstStringProp(p, ['Domovi_zdravlja_adresa', 'adresa', 'ADRESA']);
