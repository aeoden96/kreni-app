...
export const PharmaciesMap = memo(function PharmaciesMap({ show }: Props) {
  const { bounds, shouldRenderDetail } = useStaticLayerRenderGate({ variant: 'city' });
  const geoData = useStaticDatasetJson<FeatureCollection<Point>>('pharmacies.geojson', show, {
    logErrorLabel: 'pharmacies',
  });
  ...
  const featuresForCluster = useMemo(() => {
    ...
    return annotateCityClusterFeatures(valid, 'pharmacies');
  ...
  const icon = L.divIcon({
    ...
    html: `<div class="flex items-center justify-center w-7 h-7 rounded-full bg-green-500 border-2 border-white shadow-lg text-[13px]">💊</div>`,
    iconAnchor: [14, 14],
    iconSize: [28, 28],
  });
  ...
  return (
    <>
      {visibleFeatures.map((f, i) => {
        ...
        const titleStr = firstStringProp(p, ['naziv', 'NAZIV']) || 'Ljekarna';
