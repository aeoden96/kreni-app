...
it('propagates workerTimestamp and cacheStatus from fetchRealtimeFeed to the store', async () => {
  const { fetchRealtimeFeed } = await import('../utils/realtime') as any;

  fetchRealtimeFeed
    .mockResolvedValueOnce({ feed: { entity: [], header: { timestamp: 0 } }, metadata: { workerTimestamp: '2026-03-14T20:45:10Z', cacheStatus: 'HIT' } })
    .mockResolvedValueOnce({ feed: { entity: [], header: { timestamp: 0 } }, metadata: { workerTimestamp: null, cacheStatus: null } });
